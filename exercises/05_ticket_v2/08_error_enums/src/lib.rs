...
fn easy_ticket(title: String, description: String, status: Status) -> Ticket {
    ...
    if let Ok(ticket) = ticket {
        ticket
    } else if let Err(TicketNewError::TitleError { message }) = ticket {
        panic!("{}", message);
    } else {
        Ticket {
            title,
...
impl Ticket {
    pub fn new(
        ...
        if title.is_empty() {
            return Err(TicketNewError::TitleError{ message: String::from("Title cannot be empty") });
        }
        if title.len() > 50 {
            return Err(TicketNewError::TitleError{ message: String::from("Title cannot be longer than 50 bytes") });
        }
        if description.is_empty() {
            return Err(TicketNewError::DescriptionError{ message: String::from("Invalid description") });
        }
