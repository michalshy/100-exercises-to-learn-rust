...
pub fn sum(v: Vec<i32>) -> i32 {
    let mut v_one = v;
    let v_two = v_one.split_off(v_one.len() / 2);
    let sum_one: JoinHandle<i32> = thread::spawn(move || {
        v_one.iter().sum()
    });
    let sum_two: JoinHandle<i32> = thread::spawn(move || {
        v_two.iter().sum()
    });
    sum_one.join().unwrap() + sum_two.join().unwrap()
}
